...
from productos_manager import cargar_productos, reducir_stock_al_comprar, obtener_sucursales
from compras_manager import registrar_compra
...
class ClienteFrame(ctk.CTkFrame):
    ...
    def limpiar_detalle(self):
        ...
        self.listado.configure(state="normal")
        self.listado.delete("0.0", "end")
        self.listado.configure(state="disabled")
        self.label_imagen.configure(image=None, text="(sin imagen)")
        self.productos_mostrados = []
    ...
    def comprar(self):
        ...
        ok, msg = reducir_stock_al_comprar(categoria, nombre, sucursal, cantidad)
        if not ok:
            messagebox.showerror("Error", msg)
            return
        registrar_compra(usuario, categoria, nombre, cantidad, sucursal, metodo, total)
